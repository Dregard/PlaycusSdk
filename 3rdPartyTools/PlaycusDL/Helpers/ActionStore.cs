...
namespace PlaycusDL {
    ...
    internal class ActionStore {
        ...
        virtual internal void Clear() {
            ...
            salt = new byte[32];
            PlayerPrefs.DeleteKey(PDL.PF_KEY_ACTIONS_SALT);
        }

        private void InitialiseSalt() {
            if (!PlayerPrefs.HasKey(PDL.PF_KEY_ACTIONS_SALT)) {
                new RNGCryptoServiceProvider().GetNonZeroBytes(salt);
                PlayerPrefs.SetString(
                    PDL.PF_KEY_ACTIONS_SALT,
                    Convert.ToBase64String(salt));
            } else if (salt.All(e => e == 0)) {
                salt = Convert.FromBase64String(
                    PlayerPrefs.GetString(PDL.PF_KEY_ACTIONS_SALT));
            }
        }
