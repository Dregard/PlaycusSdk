...
namespace PlaycusDL {
    ...
    internal class EventTrigger : IComparable<EventTrigger>{
        ...
        internal EventTrigger(PDLBase pdl, int index, JSONObject json, ExecutionCountManager executionCountManager){
            this.pdl = pdl;
            this.index = index;
        ...
        internal virtual bool Evaluate(GameEventPDL evnt){
            ...
            if (result){
                ...
                runs++;
                var eventTriggeredActionEvent = new GameEventPDL("pdlEventTriggeredAction")
                    .AddParam("pdlEventTriggeredCampaignID", campaignId)
                    .AddParam("pdlEventTriggeredCampaignPriority", priority)
                    .AddParam("pdlEventTriggeredVariantID", variantId)
                    .AddParam("pdlEventTriggeredActionType", GetAction())
                    .AddParam("pdlEventTriggeredSessionCount", runs);

                if (campaignName != null){
                    eventTriggeredActionEvent.AddParam("pdlEventTriggeredCampaignName", campaignName);
                }

                if (variantName != null){
                    eventTriggeredActionEvent.AddParam("pdlEventTriggeredVariantName", variantName);
                }

                pdl.RecordEvent(eventTriggeredActionEvent);
